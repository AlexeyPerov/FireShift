...
class MockSupportRepository extends SupportRepository {
  ...
  Future<SupportThread> addThreadMessage(
    ...
    if (ticket == null) {
      throw ('Unable to find ticket with id $id');
    }

    var newTicket = SupportThread.clone(ticket);
    newTicket.contents.messages.add(SupportMessage(
        authorId: senderId, contents: response, time: DateTime.now()));

    return Future.value(newTicket);
  }
  ...
  Future<SupportThreadInfo> archive(String id, bool archive) {
    ...
    var info = ticket.info.copy(archived: archive);
